...
fn run_sidecar_cli(
    ...
    let (mut rx, _child) = sidecar
        .spawn()
        .map_err(|err| format!("spawn sidecar cli: {err}"))?;
...
fn install_builtin_plugins(app: &AppHandle, app_root: &Path) {
    ...
    for (plugin_id, display_name) in builtin {
        ...
        match run_sidecar_cli(
            ...
            Ok(()) => {
                println!("[desktop] plugin installed: {plugin_id}");
            }
            Err(e) => {
                eprintln!("[desktop] failed to install plugin {plugin_id}: {e}");
            }
        }
    }
